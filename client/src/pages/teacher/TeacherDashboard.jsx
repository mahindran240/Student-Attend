    ...
    <div className="page-shell">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="page-subtitle">Manage students, attendance, QR sessions, reports, and leave requests.</p>
        </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_360px]">
        ...
        <section className="panel">
          <h2 className="section-title">QR Attendance</h2>
          <button className="btn-primary mt-4 w-full" onClick={generateQr}><QrCode size={18} /> Generate QR</button>
          <div className="surface-muted mt-4 grid min-h-48 place-items-center">
            {qr ? <img src={qr} alt="QR attendance code" className="h-40 w-40 rounded-2xl bg-white p-2 shadow-md" /> : <QRCodeSVG value="Smart Attendance QR Ready" size={160} />}
          </div>
        </section>
      </div>
      ...
      <section className="panel">
        <h2 className="section-title mb-4">Attendance Reports</h2>
        <DataTable
          rows={records}
          columns={[
            ...
            { key: "actions", label: "Actions", render: (row) => <button className="rounded-xl p-2 text-red-600 transition hover:bg-red-50 hover:text-red-700 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-red-100 dark:hover:bg-red-950/40 dark:focus-visible:ring-red-900/50" onClick={() => removeAttendance(row._id)} aria-label="Delete attendance"><Trash2 size={18} /></button> }
          ]}
        />
      </section>

      <section className="panel">
        <h2 className="section-title mb-4">Approve Leave Requests</h2>
        <div className="grid gap-3">
          {(leaves.data || []).map((leave) => (
            <div className="surface-muted flex flex-wrap items-center justify-between gap-3" key={leave._id}>
              <div>
